...
class MiniPcMavrosOffboard(Node):
    ...
    def _estimate_ready(self) -> bool:
        if not (self._got_pose and self._got_odom):
            return False
    ...
    def _ensure_target_pose(self) -> None:
        ...
        if abs(self.target_z_m) > 1e-6:
            target.pose.position.z = current.position.z + delta_z_enu
        elif self.arm_only:
            target.pose.position.z = current.position.z
        else:
            target.pose.position.z = current.position.z + self.takeoff_height_m
        ...
        self.get_logger().info(
            ...
            f"x={target.pose.position.x:.2f}, y={target.pose.position.y:.2f}, z={target.pose.position.z:.2f}"
        )
    ...
    def _publish_target_pose(self) -> None:
        ...
        msg.position.z = float(self._target_pose.pose.position.z)
    ...
    def _timer_cb(self) -> None:
        ...
        if self.phase == Phase.WAIT_FOR_LOCAL_POSE:
            self._log_phase("Waiting for MAVROS local pose before arming")
