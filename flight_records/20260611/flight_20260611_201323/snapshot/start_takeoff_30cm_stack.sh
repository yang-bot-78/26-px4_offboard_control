...
open_window() {
  local title="$1"
  local cmd="$2"
  gnome-terminal --title="$title" -- bash -lc "export FLIGHT_TIMESTAMP='${flight_timestamp}'; export FLIGHT_RUN_DIR='${flight_run_dir}'; $cmd; exec bash" &
}
...
open_window "Takeoff 30cm" \
  "sleep $((driver_delay + fastlio_delay + mavros_delay + bag_delay)); cd '${HOME}/ws_offboard_control' && ./run_takeoff_30cm_hold.sh"
