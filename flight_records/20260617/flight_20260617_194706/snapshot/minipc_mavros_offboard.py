...
class MiniPcMavrosOffboard(Node):
    ...
    def _publish_offboard_land_pose(self) -> None:
        ...
        z_start = float(self._offboard_land_start_z)
        z_end = float(self._reference_pose.pose.position.z)
        if z_start >= z_end:
            z_cmd = max(z_end, z_start - self.offboard_land_speed_mps * elapsed)
        else:
            z_cmd = min(z_end, z_start + self.offboard_land_speed_mps * elapsed)
        msg.position.z = float(z_cmd)
    ...
    def _offboard_land_completed(self) -> bool:
        ...
        return abs(current_z - ground_z) <= self.target_reached_tolerance_m
    ...
    def _timer_cb(self) -> None:
        ...
        if self.phase == Phase.HOLD:
            ...
            if not self.state.armed or not self._offboard_active():
                if self.state.armed and self.land_on_offboard_loss:
        ...
        if self.phase == Phase.OFFBOARD_LAND:
            ...
            if self._offboard_land_completed():
                self._landed_hold_start_ns = self.get_clock().now().nanoseconds
                self.phase = Phase.LANDED_HOLD
            return
